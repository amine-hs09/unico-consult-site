(function() {
  if (localStorage.getItem('unico-cookies-accepted')) return
  ...
  banner.innerHTML = `
    <div style="position:fixed;bottom:0;left:0;right:0;z-index:200;padding:16px 24px;background:rgba(30,27,75,0.95);backdrop-filter:blur(16px);display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap">
      ...
      <div style="display:flex;gap:10px;flex-shrink:0">
        ...
        <button onclick="document.getElementById('cookie-banner').remove();localStorage.setItem('unico-cookies-accepted','1')" style="padding:8px 20px;border-radius:8px;border:1px solid rgba(255,255,255,0.2);background:transparent;color:rgba(255,255,255,0.7);font-weight:500;font-size:13px;cursor:pointer">Fermer</button>
      </div>
    </div>
  `
